...
function EngInventory_Options_InitWindow()
	...
	EngInventory_OptsFrame:SetHeight( ENGINVENTORY_OPTIONS_FRAME_HEIGHT );
		...
		EngInventory_OptsFrame:EnableMouseWheel(true);
		EngInventory_OptsFrame:SetScript("OnMouseWheel", function(_, delta)
			local currentValue = EngInventory_OptsFrame_ScrollBar:GetValue();
			local newValue = currentValue - delta;
			newValue = math.max(1, math.min(EngInventory_Config_MaxScroll, newValue));
			EngInventory_OptsFrame_ScrollBar:SetValue(newValue);
		end);
